    ...
    <form method="POST" action="/updatecompanyinfo">
     {{ csrf_field() }}
        ...
        <div>
            ...
            <input type="text" id="cname" name="cname" value="{{$details->name}}" class="w-full text-gray-400 rounded-lg"><br>
        </div>
        <div>
            <label for="email" class="pb-2 text-gray-700">email:</label><br>
            <input type="text" id="email" name="email" value="{{$details->email}}email" class="w-full text-gray-400 rounded-lg"><br> 
        </div>
        <div>
            <label for="website" class="pb-2 text-gray-700">website:</label><br>
            <input type="text" id="website" name="website" value="{{$details->website}}" class="w-full text-gray-400 rounded-lg"><br>
        </div>
        <div>
            <label for="logo" class="pb-2 text-gray-700">Select logo:</label><br>
            <input  class="block w-full h-8 pb-2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" type="file" id="image" accept="image/*"  name="image" value="{{$details->logo}}"><br>
        </div> 
